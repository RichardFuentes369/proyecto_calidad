...
class ModuleTranslateLoader {
    ...
    fetchTranslation(language, { translateError, version, headers }, { pathTemplate, baseTranslateUrl, translateMap }) {
        ...
        return this.http.get(path, { headers }).pipe(map((translation) => (translateMap ? translateMap(translation) : translation)), this.catchError(cleanedPath, translateError));
    }
    fetchTranslationForModule(language, { disableNamespace, lowercaseNamespace, translateError, version, headers }, { pathTemplate, baseTranslateUrl, moduleName, namespace, translateMap, headers: moduleHeaders }) {
        ...
        const cleanedPath = concatJson(template.replace(PATH_TEMPLATE_REGEX, (_, m1) => pathOptions[m1] || '')).replace(PATH_CLEAN_REGEX, '$1');
        const path = version ? `${cleanedPath}?v=${version}` : cleanedPath;
        return this.http.get(path, { headers: moduleHeaders || headers }).pipe(map((translation) => {
            return translateMap
